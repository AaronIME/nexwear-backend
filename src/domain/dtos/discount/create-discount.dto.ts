export class CreateDiscountDto {
  ...
  static create(object: { [key: string]: any }): [string | undefined, CreateDiscountDto?] {
    ...
    if (percentage === undefined || percentage === null) return ['Percentage property is required'];
    if (typeof percentage !== 'number' || percentage < 0 || percentage > 100) {
      return ['Percentage must be a number between 0 and 100'];
    }

    if (!startDate) return ['StartDate property is required'];
    if (!endDate) return ['EndDate property is required'];
    ...
    if (parsedEnd <= parsedStart) return ['EndDate must be after StartDate'];
